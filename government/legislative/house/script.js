...
if (params.has('member')) {
    ...
    req.onload = function () {
        ...
        document.querySelector('.jumbotron p.lead').innerHTML = `${data.state} District ${data.district} (${data.partyHistory[data.partyHistory.length - 1].partyName})`;
    ...
    req.onload = function () {
        ...
        for (const member of data.members)
            if (member.terms && member.terms.item[member.terms.item.length - 1].chamber == 'House of Representatives' && !member.terms.item[member.terms.item.length - 1].endYear) {
        ...
        members = members.concat(data.members.filter(member => { return member.terms && member.terms.item[member.terms.item.length - 1].chamber == 'House of Representatives' && !member.terms.item[member.terms.item.length - 1].endYear }));
        if (finishedLoading) {
